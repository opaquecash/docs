...
**ONS names (spec `ONS.md`).** Subnames of the Opaque Name Service parent resolve
mirror-first: with `solana` configured, the SDK derives the mirror PDA from
`keccak256(name)` and reads **one Solana account — no Ethereum RPC**. Without `solana`
