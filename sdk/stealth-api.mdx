...
| --- | --- | --- |
| meta-address, 66- or 98-byte (optionally `st:opq:`-prefixed) | Validated and passed through | `"meta-address"` |
...
**ONS names ([ONS.md](https://github.com/opaquecash/spec/blob/main/ONS.md)).** Subnames
...
[naming guide](/guides/ons-naming) for the consistency model):
...
<Note>
  ...
  [Starknet](/concepts/starknet).
</Note>

---

### `buildStarknetStealthSend(params)`
...
const starknetSend = await client.buildStarknetStealthSend({
  recipient,   // anything resolveRecipient accepts
  amount,      // STRK amount
});
// Unsigned transfer + announce calls for the connected Starknet account to broadcast.
```

Requires the `starknet: {}` client option and `@opaquecash/stealth-chain-starknet`. See
[Starknet](/concepts/starknet) for the full flow and caveats.
...
<Note>
  Include `"starknet"` (Starknet Sepolia, testnet-preview) only when the client was
...
For a stealth address holding a token but no gas, use `buildGaslessTokenSweep` and submit
...
(see [Gasless token sweep](/guides/scan-and-sweep#gasless-token-sweep)). The gasless
...
```ts
const starknetSweep = client.buildStarknetSweep({
  output,        // a Starknet UnifiedOwnedOutput
  destination,   // fresh Starknet address
  amount,
});
// Broadcast with the connected Starknet account (starknet.js EthSigner).
```

See [Starknet](/concepts/starknet).
