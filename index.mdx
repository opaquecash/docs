...
flowchart LR
  ...
  Client --> SOL["Solana\nRegistry · Announcer · PSR"]
  Client --> STARK["Starknet\nRegistry · Announcer · PSR verify"]
...
| Starknet Sepolia | `0x534e` (SN) | Stealth, PSR verify/gate, reputation verifier (testnet preview) |

<Note>
  Starknet Sepolia is the newest chain and ships at testnet-preview maturity. It participates as a native stealth + scan chain with on-chain PSR proof verification and gating (`PsrGate`) — schema issuance and Wormhole (UAB) relay stay on Ethereum and Solana. See [Starknet](/concepts/starknet) for the full flow and caveats.
</Note>
