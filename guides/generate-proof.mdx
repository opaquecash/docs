...
## Step 2 — Reconstruct stealth signing key
...
## Step 3 — Build action scope

```ts
import { buildActionScope, externalNullifierFromScope } from "@opaquecash/opaque";

const scope = buildActionScope(11155111, "my-app", "premium-gate");
const externalNullifier = externalNullifierFromScope(scope);
```

Or use static helpers on the client:

```ts
const scope = OpaqueClient.buildReputationActionScope(11155111, "my-app", "premium-gate");
const externalNullifier = OpaqueClient.reputationExternalNullifierFromScope(scope);
```
...
// proofData: { proof, nullifier, attestationId, ... }
```

## Step 5 — Fetch valid Merkle root
