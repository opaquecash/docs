...
const client = await OpaqueClient.create({
  ...
  starknet: {},                 // enable Starknet (Sepolia) scan + balances
});
```
...
Starknet sweeps don't go through `sweep`. Build the calls with `buildStarknetSweep` and let
...
const starknetSweep = client.buildStarknetSweep({
  output: starknetOutput,
  destination: "0xFreshStarknetAddress...",
  amount: balance,              // STRK, raw
});
// The app broadcasts starknetSweep with starknet.js (EthSigner, v10+).
```

See [Starknet](/concepts/starknet) for the full flow and caveats.
...
const sweep = await client.buildGaslessTokenSweep({
  ...
  token: "EPjF...USDCmint",
  destination: "FreshOwner...base58",
  fee: 50_000n,                       // transferred to the relayer's ATA
  deadline: 0n,                       // unused on Solana
  feePayer: "RelayerPubkey...base58", // read it from the relayer's /v1/sweep/info
});
```

### Submitting to a relayer node

An `opaque-relayer` node serves the sweep directly over HTTP — no job, bid, or payload
...
const gateway = { baseUrl: "http://relayer.example:8787" };
const info = await getSweepInfo(gateway);   // { chains: [{ chain, operator, forwarder? }] }

const { tx } = await postGaslessSweep(gateway, gaslessSweepSubmission(sweep));
```

The node fronts the gas, is reimbursed the in-token `fee`, and returns the confirmed
