...
const { txHash, uid, stealthAddressHash } = await client.issueAttestation("ethereum", {
  schemaId,
  recipient: recipientMetaAddressHex,   // meta-address hex
  fieldValues: {
    passed: "true",
    jurisdiction: "US",
  },
...
The announcement uses the V2 `0xB2` metadata layout, so recipients discover it with
...
These helpers encode the older V1 `0xA7` attestation id marker. Keep them only for legacy
